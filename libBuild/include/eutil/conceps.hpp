...
namespace util
{
    ...
    template<typename T> concept IsCharArray_v = std::is_array_v<T> && (std::is_same_v<typename std::remove_extent_t<T>, char> || std::is_same_v<typename std::remove_extent_t<T>, wchar_t> || std::is_same_v<typename std::remove_extent_t<T>, char16_t> || std::is_same_v<typename std::remove_extent_t<T>, char32_t>);
    template<typename T> concept IsConstCharArray_v = std::is_array_v<T> && (std::is_same_v<typename std::remove_extent_t<T>, const char> || std::is_same_v<typename std::remove_extent_t<T>, const wchar_t> || std::is_same_v<typename std::remove_extent_t<T>, const char16_t> || std::is_same_v<typename std::remove_extent_t<T>, const char32_t>);
}